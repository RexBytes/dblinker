from .dbconfig_manager import DBConfigManager
...
def cli_dbconfig_subparser(subparsers):
    ...
    create_parser.add_argument('--configfilepath', required=True, help='Path to where you want your configuration file created.')
    ...
    test_parser.add_argument('--configfilepath', required=True, help='Path to the configuration file you want to test.')
...
def dbconfig_create_handler(args):
    ...
    configfilepath = Path(args.configfilepath)
    if configfilepath.exists():
        ...
        dbconfig_manager.write_config_template_text(configfilepath, config_template_text)


def dbconfig_test_handler(args):
    dbconfig_manager = args.dbconfig_manager
    dbconfig_manager.test_connection(args.configfilepath)
